...
I2C_MemInfoHandle I2C_MEM_open(Uint32 i2cPeripheralNum, Uint32 i2cOwnAddr, Uint32 i2cMemAddr)
{
  ...
  i2cCfg.ownAddr    = i2cOwnAddr;
  i2cCfg.prescalar  = 6;
  i2cCfg.i2cclkh    = 63;
  i2cCfg.i2cclkl    = 63;
  
  // Open I2C peripheral
  hI2CMemInfo->hI2CInfo = I2C_open(i2cPeripheralNum,I2C_ROLE_MASTER,I2C_MODE_MANUAL_STOP,&i2cCfg);
  if (hI2CMemInfo->hI2CInfo == NULL)
    return NULL;
  
  // Set the current slave address
  hI2CMemInfo->hI2CInfo->slaveAddr = i2cMemAddr;

  // FIXME: Detect addr width, page width, size
  hI2CMemInfo->addrWidth = 16;
  hI2CMemInfo->pageSize = 32;
  hI2CMemInfo->memorySize = 8192;
...
Uint32 I2C_MEM_readBytes(I2C_MemInfoHandle hI2CMemInfo, Uint32 addr, Uint32 byteCnt, Uint8 *dest)
{
  ...
  if ((addr+byteCnt)>hI2CMemInfo->memorySize)
    return E_FAIL;
...
Uint32 I2C_MEM_writeBytes(I2C_MemInfoHandle hI2CMemInfo, Uint32 addr, Uint32 byteCnt, Uint8 *src)
{
  ...
  cycCnt = 0;
	...
	{
		k = ( (byteCnt-i)>hI2CMemInfo->pageSize ) ? hI2CMemInfo->pageSize : (byteCnt-i);
