...
I2C_InfoHandle I2C_open(Uint32 i2cPeripheralNum, I2C_Role role, I2C_Mode mode, I2C_Config *config)
{
  ...
  I2C_assertReset(hI2CInfo);
	...
	// Set Own Address
	I2C->ICOAR = config->ownAddr;
	 
	// Set Default I2C High and Low Clock Hold
	I2C->ICPSC  = (config->prescalar - 1) & 0xFF;
	I2C->ICCLKL = (config->i2cclkl - 1) & 0xFF;
	I2C->ICCLKH = (config->i2cclkh - 1) & 0xFF;
...
RESTART_WRITEBYTES:
  ...
  while (currByteCnt)
  {
    ...
    if (i2cstr & (I2C_ICSTR_NACK_MSK | I2C_ICSTR_ARDY_MSK))
	  {
		  I2C_assertReset(hI2CInfo);
      I2C_releaseReset(hI2CInfo);
	    goto RESTART_WRITEBYTES;
    }

    if (i2cstr & I2C_ICSTR_ICXRDY_MSK)
    {
