...
namespace TIBootAndFlash
{
  ...
  partial class Program
  {
    ...
    private static void DispHelp()
    {
      ...
      Console.Write("\tsfh_"+devString+" -norflash [<Options>] <Application binary image>\n");
      Console.Write("\t\t" + "-norflash\tRestore NOR Flash with bootable application (typically U-Boot).\n");
      Console.Write("\tsfh_"+devString+" -nandflash [<Options>] <UBL binary image> <Application binary image>\n");                
      Console.Write("\t\t" + "-nandflash\tRestore NAND Flash with bootable UBL and application (typically U-Boot).\n");            
    ...
    public static void WorkerThreadStart()
    {
      ...
      try
      {
        ...
        if (!SerialIO.waitForSequence("BootMode = UART", "BootMode = N", MySP, true))
        {
