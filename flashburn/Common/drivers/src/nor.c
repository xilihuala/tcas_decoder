...
NOR_InfoHandle NOR_open(Uint32 baseCSAddr, Uint8 busWidth)
{
  ...
  hNorInfo = (NOR_InfoHandle) UTIL_allocMem(sizeof(NOR_InfoObj));
  if (!hNorInfo)
    return NULL;
  ...
  if (LOCAL_flashQueryCFI(hNorInfo) == E_PASS)
  {
    ...
    if (hNorInfo->numberRegions == 1)
    {
      hNorInfo->flashBase += (hNorInfo->flashSize>>1);
      if ( LOCAL_flashQueryCFI(hNorInfo) == E_PASS )     
      {
	      hNorInfo->flashSize >>= 1;
        hNorInfo->flashBase -= hNorInfo->flashSize;
	      hNorInfo->numberBlocks[0] >>= 1;
	    }
      else
      {
        hNorInfo->flashBase -= (hNorInfo->flashSize>>1);
  ...
  {
    DEBUG_printString("CFI query failed.\r\n");
    return NULL;
  }
    
  // Setup function pointers
  DEBUG_printString("NOR Initialization:\r\n");
